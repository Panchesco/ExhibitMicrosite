 ...
 */
 ...
 foreach($attachments as $key => $attachment) {
   if($attachment->block_id == $block_id) {
     ...
     $block_items[$key]['item_url'] = record_url($attachment->Item, 'show',[]);
     $block_items[$key]['plus'] = ( count($block_items[$key]['files']) > 1 ) ? count($block_items[$key]['files']) - 1 : 0;
 ...
 <?php foreach($block_items as $key => $item ) : if(isset($item['files'][0])) : ?>
        <div class="item-wrapper <?php echo $padding_x . " " . $padding_y; ?>"  <?php echo $style; ?>>
        <figure>
           <div class="image-wrapper">
             <?php echo item_image('thumbnail',['class' => 'lazy'],0,$item['item']);?>
             <?php if($item['plus'] !==  0):?><div class="plus">+<?php echo $item['plus'];?></div><?php endif;?>
             <a class="item-link" href="<?php echo $item['item_url'];?>"><span class="visually-hidden">
